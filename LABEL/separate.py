...
class ECGProcessor:
    def __init__(self, directory=".", output_directory="output"):
        self.directory = Path(directory)
        self.output_directory = Path(output_directory)
        self.file_pairs = {}
        
        # Create output directory and subdirectories
        self.normal_dir = self.output_directory / "normal"
        self.abnormal_dir = self.output_directory / "abnormal"
        
        # Create all required directories
        self.output_directory.mkdir(exist_ok=True)
        self.normal_dir.mkdir(exist_ok=True)
        self.abnormal_dir.mkdir(exist_ok=True)
    ...
    def scan_files(self):
        ...
        print(f"\nFound {len(self.file_pairs)} subjects:")
        for subject_id, files in self.file_pairs.items():
            print(f"Subject {subject_id}: {', '.join(files.keys())}")
    ...
    def process_subject(self, subject_id):
        ...
        # Determine if RMSSD is normal (1) or not (0)
        is_normal = 1 if ranges['range_low'] <= (info['rmssd'] * 1000) <= ranges['range_high'] else 0
        
        # Create labeled data with RMSSD and normality indicator
        labeled_data = np.append(ecg_data, is_normal)
        
        # Choose appropriate directory based on label
        save_dir = self.normal_dir if is_normal == 1 else self.abnormal_dir
        
        # Save labeled data in appropriate directory
        output_filename = save_dir / f"S{subject_id}_labeled.txt"
        np.savetxt(
            output_filename,
            labeled_data,
            fmt='%.16e',
    ...
    def print_analysis_summary(self, result):
        ...
        print(f"Classification: {'Normal' if result['is_normal'] == 1 else 'Abnormal'}")
...
if __name__ == "__main__":
    try:
        # Initialize processor with specified output directory
        processor = ECGProcessor(output_directory="output_files")
